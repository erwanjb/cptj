            ...
            connection.query(q,(error,results,fields)=>{
                if(error) throw error;
                    ...
                    for(let i = 0;i<categorie.length;i++){
                        let q2 = "SELECT video.video, video.date FROM join_vid_cat JOIN video ON join_vid_cat.id_video=video.id JOIN categorie ON join_vid_cat.id_categorie=categorie.id WHERE type='"+categorie[i]+"' ORDER BY date DESC LIMIT 0, "+nb;
                        connection.query(q2,(e,r,f)=>{
                            if(e) throw e;
                            else{
                                tab.push(r);
                                let q3 ="SELECT video.video, video.date FROM join_vid_cat JOIN video ON join_vid_cat.id_video=video.id JOIN categorie ON join_vid_cat.id_categorie=categorie.id WHERE categorie.type='"+categorie[i]+"' ORDER BY date DESC LIMIT "+nb+", 1";
            ...
            connection.query(q,(error,results,fields)=>{
                if(error) throw error;
                    ...
                    if(results.length==1){
                        ...
                        const q2="INSERT INTO video (video, date) VALUES ('"+video+"', '"+req.body.u.date+"')";
                        connection.query(q2, (e,r,f)=>{
                            if (e) throw e;
                            else{
